...
function getDeckIndex(card, search) {
...
function winner(deckSteve, deckJosh) {
  ...
  for (let i = 0; i < deckSteve.length; i++) {
    if (getDeckIndex(card, deckSteve[i]) > getDeckIndex(card, deckJosh[i]))
      steveScore++;
    else if (getDeckIndex(card, deckSteve[i]) < getDeckIndex(card, deckJosh[i]))
      joshScore++;
  }
